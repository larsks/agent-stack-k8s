...
apk add --update-cache --no-progress helm yq skopeo git aws-cli
...
if [[ "${helm_repo_pecr:-}" != "" ]] ; then
    ...
    helm login public.ecr.aws -u AWS --password "$(aws --region us-east-1 ecr-public get-login-password)"
