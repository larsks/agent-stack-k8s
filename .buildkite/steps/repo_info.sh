...
if [[ "${version}" == "${BUILDKITE_TAG#v}" ]] ; then
    ...
    helm_repo_pecr="oci://public.ecr.aws/buildkite/agent-stack-k8s/helm"
    helm_image_pecr="${helm_repo_pecr}:${version}"
    ...
    controller_image_pecr="$(buildkite-agent meta-data get controller-image-pecr)"
    helm_repo_pecr="oci://public.ecr.aws/buildkite/agent-stack-k8s-dev/helm"
    helm_image_pecr="${helm_repo_pecr}:${version}"
fi
